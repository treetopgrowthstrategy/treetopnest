...
function getTierDetails(score) {
    if (score <= 6)
        return { label: 'AI-Absent', color: '#ef4444', urgency: 'Critical Gap', summary: 'Your GTM motion has significant structural gaps vs. competitors who have already rebuilt around AI. The pipeline impact is material and compounding every quarter.', competitive: 'Companies that made the AI-native transition 12–18 months ago are now running teams 30–40% smaller while generating more qualified pipeline. That gap widens every quarter you wait.' };
    if (score <= 12)
        return { label: 'AI-Adjacent', color: '#f97316', urgency: 'Significant Gap', summary: "You're using AI tools, but your revenue motion isn't architected around them. The gap between tool adoption and AI-native transformation is where most pipeline is lost.", competitive: 'Your closest competitors are likely in the same position — or one step ahead. The window to lead this transformation is open, but it narrows as AI-native tools mature and competitors move.' };
    if (score <= 18)
        return { label: 'AI-Augmented', color: '#3b82f6', urgency: 'Optimization Opportunity', summary: 'You have a solid foundation. Your weakest 1–2 pillars are where competitors are pulling ahead. A targeted overhaul of those areas would close the remaining gap.', competitive: 'Companies at the AI-Native level compound their advantage through learning loops — each quarter their systems get better. Closing the remaining gap now prevents that compounding from widening.' };
...
function buildReportHTML(data, reportUrl) {
...
function buildProspectEmail(data, reportUrl, tier) {
    const firstName = data.name.split(' ')[0];
...
function buildBillNotifyEmail(data, tier, reportUrl) {
...
async function logToAirtable(data, tier, reportUrl, slug) {
    if (!AIRTABLE_API_KEY) {
...
;
//# sourceMappingURL=quiz-submit.js.map