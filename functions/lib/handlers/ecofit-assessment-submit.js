...
function buildSlug(company, date) {
...
// ─── GITHUB PUBLISH ───────────────────────────────────────────────────────────
async function pushReportToGitHub(slug, html) {
    if (!GITHUB_TOKEN) {
    ...
    const body = { message: `Add Ecofit report: ${slug}`, content, branch: 'main' };
    if (sha)
        body.sha = sha;
...
function buildProspectEmail(data, reportUrl) {
    const firstName = data.name.split(' ')[0];
    const riskRange = `${fmt(data.riskLow)}–${fmt(data.riskHigh)}`;
    const a = data.answers;
    ...
    let hook = '';
    if (a.tracking === 'none')
        hook = 'Your facility is running without any formal equipment tracking — that\'s the single largest driver of OpEx waste we see.';
    else if (a.maintenance === 'reactive')
        hook = 'With most of your maintenance spend going toward unplanned repairs, you\'re absorbing a 3–5x cost premium over best-in-class operators.';
    else if (a.age === '10+')
        hook = 'A 10+ year fleet without utilization data means your next capex cycle is driven by age and complaints, not performance.';
    else if (a.data_confidence === 'none')
        hook = 'Operating without equipment ROI data means every budget conversation is built on estimates — not a position you want to defend.';
    else
        hook = `Based on your profile across ${data.networkLabel}, there\'s a clear and closable gap between where you are and where best-in-class operators operate.`;
...
function buildBillNotifyEmail(data, reportUrl) {
    const riskRange = `${fmt(data.riskLow)}–${fmt(data.riskHigh)}`;
    const a = data.answers;
    // Signal strength summary for quick triage
    const signals = [];
    if (a.tracking === 'none' || a.tracking === 'manual')
        signals.push('❗ No / manual tracking');
    if (a.maintenance === 'reactive')
        signals.push('❗ Reactive maintenance');
    if (a.age === '10+' || a.age === '6-10')
        signals.push(`⚠️ Aging fleet (${label('age', a.age)})`);
    if (a.data_confidence === 'none' || a.data_confidence === 'low')
        signals.push('❗ No / low ROI data confidence');
    if (a.impact === 'known-pain' || a.impact === 'frequently')
        signals.push('⚠️ Frequent downtime impact');
    if (!['1'].includes(a.locations))
        signals.push(`📍 ${label('locations', a.locations)}`);
...
// ─── AIRTABLE ─────────────────────────────────────────────────────────────────
async function logToAirtable(data, reportUrl, slug) {
    if (!AIRTABLE_API_KEY) {
    ...
    if (reportUrl)
        fields['Report URL'] = reportUrl;
    if (slug)
        fields['Report Slug'] = slug;
...
async function handle(request) {
    try {
        ...
        let reportUrl = null;
        try {
            reportUrl = await pushReportToGitHub(slug, reportHtml);
        }
        catch (e) {
...
;
//# sourceMappingURL=ecofit-assessment-submit.js.map