#!/usr/bin/env python3
"""
treetop_niche_expander.py

Idempotent generator for the "AI for {niche}" thin-content pages.
Each niche supplies rich, industry-specific content as data; this script
stamps the shared standalone-HTML skeleton (matching the existing page
design system) and auto-builds the FAQPage schema from the visible FAQ so
the two never drift apart.

Run from repo root:  python3 treetop_niche_expander.py
Writes to src/pages/{slug}.astro  (only the slugs present in NICHES below).
"""
import html
import json
import os
...
# ---- shared CSS (kept as a plain constant: it contains { } and must not be f-formatted) ----
STYLE = """<style>
...
<div><div class="cta-text">Want help designing AI workflows for your business?</div><div class="cta-sub">The $1,500 AI Audit produces a written roadmap in 5 business days.</div></div>
...
# ---------------------------------------------------------------------------
# NICHE DATA  (batch 1: food / beverage / events / beauty / wellness / fitness)
# ---------------------------------------------------------------------------
NICHES = {}

if __name__ == "__main__":
    # NICHES is populated by treetop_niche_data.py to keep this file readable.
    from treetop_niche_data import NICHES as _N
    NICHES.update(_N)
    main()
