// Vercel-native serverless function for Ecofit assessment submission.
...
// src/pages/api/ are not served. Matches the pattern of api/quiz-submit.ts.
...
function getRecommendations(data: AssessmentPayload): { title: string; detail: string }[] {
  ...
  if (a.tracking === 'none' || a.tracking === 'manual') {
    recs.push({
      ...
      detail: `Right now your team is operating blind. Ecofit's connected layer gives you live visibility across every asset in ${data.networkLabel}. Operators who make this move first in their market consistently outperform on OpEx within 12 months.`
    });
  } else if (a.tracking === 'tickets') {
    recs.push({
      title: 'Upgrade from reactive tracking to predictive intelligence',
      detail: 'Service tickets tell you what already broke. Ecofit surfaces the signals before failure so your maintenance team is working a schedule, not a crisis queue. The difference typically shows up as a 30-40% reduction in unplanned service spend.'
    });
  }
  ...
  if (a.maintenance === 'reactive' || a.maintenance === 'unknown') {
    recs.push({
      ...
      detail: `${a.maintenance === 'reactive' ? 'Over half your maintenance budget is unplanned' : "Without tracking your maintenance split, you're likely absorbing significant reactive premium"}. Industry data shows reactive repairs cost 3-5x more than planned maintenance for the same work.`
    });
  }
  ...
  if (a.age === '6-10' || a.age === '10+') {
    recs.push({
      ...
      detail: `A ${a.age === '10+' ? '10+ year' : '6-10 year'} fleet without utilization data means your capex decisions are based on age and complaints, not performance. Ecofit shows you which equipment is still earning its floor space.`
    });
  }
  ...
  if (multiLoc && recs.length < 3) {
    recs.push({
      ...
      detail: `With ${data.networkLabel}, your most powerful insight is what the outliers reveal about the whole network. Ecofit surfaces which locations are performing above or below network average on every metric.`
    });
  }
  ...
  if ((a.data_confidence === 'low' || a.data_confidence === 'none') && recs.length < 3) {
    recs.push({
      ...
      detail: 'Walking into a budget review without reliable equipment ROI data means defending spend with anecdotes. Ecofit gives you the number so every capex decision is backed by data your CFO can interrogate.'
    });
  }
  ...
  if (recs.length < 3) {
    recs.push({
      ...
      detail: `Ecofit gives your team a real-time view of every machine - uptime, usage, and performance - across ${data.networkLabel}, every day.`
    });
  }
...
<div style="background:rgba(20,25,31,0.95);border-bottom:1px solid rgba(132,188,65,0.12);position:sticky;top:0;z-index:50;backdrop-filter:blur(12px)">
  <div style="max-width:780px;margin:0 auto;padding:0 24px;height:58px;display:flex;align-items:center;justify-content:space-between">
    <span style="font-family:Raleway;font-weight:700;color:#84BC41;font-size:18px">ecofit</span>
    <span style="font-family:'Work Sans',sans-serif;font-size:10px;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;color:rgba(242,243,248,0.35)">Facility Intelligence Report</span>
  </div>
</div>
<div class="container">
  <div style="padding:60px 0 48px;border-bottom:1px solid rgba(132,188,65,0.12)">
    ...
    <h1 style="font-family:'Raleway',sans-serif;font-weight:700;font-size:clamp(2rem,6vw,3.2rem);line-height:1.1;color:#F2F3F8;margin-bottom:10px;letter-spacing:-0.015em">Facility Intelligence<br><span style="color:#84BC41">Report</span></h1>
    <p style="font-family:'Work Sans',sans-serif;font-size:13px;color:rgba(242,243,248,0.4);margin-bottom:40px">${data.company}${data.title ? ` - ${data.title}` : ''} - ${date}</p>
    <div style="background:#2F3535;border:1px solid rgba(132,188,65,0.2);border-left:4px solid #84BC41;padding:36px 40px;margin-bottom:32px">
      ...
      <div style="font-family:'Work Sans',sans-serif;font-size:14px;color:#9699A2;line-height:1.6">Based on your facility profile across ${data.networkLabel}.</div>
    </div>
    ...
    <p style="font-family:'Work Sans',sans-serif;font-size:15px;color:#9699A2;line-height:1.75;max-width:680px">${firstName}, these are costs your network is already absorbing, invisibly. The operators who have closed this gap with Ecofit don't just reduce spend. They make faster, smarter decisions at the board level.</p>
  </div>
  ...
  <div style="background:#14191f;border:1px solid rgba(132,188,65,0.2);border-top:3px solid #84BC41;padding:52px;text-align:center">
    <h2 style="font-family:'Raleway',sans-serif;font-weight:700;font-size:clamp(1.6rem,4vw,2.4rem);color:#F2F3F8;line-height:1.2;margin-bottom:14px">Want to see what your network is really telling you?</h2>
    <p style="font-family:'Work Sans',sans-serif;font-size:14px;color:#9699A2;margin-bottom:32px;line-height:1.75;max-width:480px;margin-left:auto;margin-right:auto">30 minutes with an Ecofit analyst. No commitment. No IT team required.</p>
    <a href="${BOOKING_LINK}" style="display:inline-flex;align-items:center;gap:8px;background:#84BC41;color:#14191f;font-family:'Work Sans',sans-serif;font-weight:600;font-size:15px;padding:16px 36px;text-decoration:none;border-radius:4px">Book a Demo</a>
  </div>
</div>
</body>
</html>`;
...
async function pushReportToGitHub(slug: string, html: string): Promise<string | null> {
  ...
  try {
    ...
    if (check.ok) { const ex: any = await check.json(); sha = ex.sha; }
  } catch (_) {}
...
function buildProspectEmail(data: AssessmentPayload, reportUrl: string | null): string {
  ...
  else if (a.maintenance === 'reactive') hook = 'With most of your maintenance spend going toward unplanned repairs, you are absorbing a 3-5x cost premium over best-in-class operators.';
  ...
  else if (a.data_confidence === 'none') hook = 'Operating without equipment ROI data means every budget conversation is built on estimates.';
  else hook = `Based on your profile across ${data.networkLabel}, there is a clear and closable gap between where you are and where best-in-class operators operate.`;
...
<div style="max-width:580px;margin:40px auto;background:#14191f;overflow:hidden;border:1px solid rgba(132,188,65,0.15)">
  ...
  <div style="padding:40px 32px">
    ...
    <p style="font-size:13px;color:#9699A2;line-height:1.75;margin-bottom:28px">If you would like to talk through what this means for ${data.company}, I am happy to do a 30-minute no-commitment demo.</p>
  ...
  <div style="background:#2F3535;border-top:1px solid rgba(132,188,65,0.1);padding:16px 32px">
    <span style="font-size:11px;color:rgba(242,243,248,0.25)">Ecofit Networks Inc.</span>
  </div>
</div>
</body></html>`;
}
...
function buildBillNotifyEmail(data: AssessmentPayload, reportUrl: string | null): string {
  ...
  if (a.age === '10+' || a.age === '6-10') signals.push(`Aging fleet (${label('age', a.age)})`);
  if (a.data_confidence === 'none' || a.data_confidence === 'low') signals.push('No / low ROI data confidence');
  if (a.impact === 'known-pain' || a.impact === 'frequently') signals.push('Frequent downtime impact');
...
<div style="background:white;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden">
  <div style="background:#14191f;padding:16px 24px">
    ...
    <span style="font-size:12px;color:rgba(242,243,248,0.5);margin-left:12px">New Assessment Lead</span>
  </div>
...
export default async function handler(req: any, res: any) {
  ...
  try {
    let data: AssessmentPayload = req.body;
    if (typeof data === 'string') {
      try { data = JSON.parse(data); } catch { data = {} as AssessmentPayload; }
    }
    ...
    // 1. Build report HTML and push to GitHub
    ...
    try {
      await sendEmail([BILL_EMAIL, DAVE_EMAIL], `New Ecofit Assessment: ${data.name} at ${data.company} - ${fmt(data.riskLow)}-${fmt(data.riskHigh)} at risk`, buildBillNotifyEmail(data, reportUrl));
