...
//
// This lives in /api (not under an underscore-prefixed name) on purpose:
// Vercel compiles each api/*.ts to a separate runtime ESM module and EXCLUDES
// underscore-prefixed files from the deployment, which makes `import ... from
// './_cmo-guards'` fail with ERR_MODULE_NOT_FOUND at runtime. A regular api
// file is included in the function bundle (same as cmo-free-report, which is
// already imported by cmo-free-qualify). The default export below is an inert
// 404 so the incidental /api/cmo-guards route does nothing; the real value is
// the named exports imported by the handlers.
//
// What it provides:
...
// This file is a shared library, but living in /api means Vercel also exposes
// it as a route. Return 404 so that route is a no-op; the handlers use the
// named exports above.
