// Vercel-native serverless function for Claude Training inquiries.
...
const FLD_TRAINING  = 'fldyLDwiA3q95fvEX'; // Claude Training
...
async function logToAirtable(data: any) {
  ...
  if (!res.ok) console.error('Airtable error:', res.status, await res.text());
}
...
<div style="max-width:560px;margin:40px auto;background:#050D05;overflow:hidden;">
  ...
  <div style="padding:40px 32px;">
    ...
    <p style="font-size:14px;color:#8FAF8F;line-height:1.75;margin:0 0 28px;">I'll follow up within one business day to discuss what would work best for ${data.company}${data.format ? `. It looks like you're interested in the <strong style="color:#F0FFF0;">${data.format}</strong>` : ''}. No commitment, just a conversation.</p>
    ...
    <p style="font-size:13px;color:#8FAF8F;line-height:1.7;margin:0;">Best,<br/>Bill<br/><span style="color:#4A6A4A;">bill@treetopgrowthstrategy.com</span></p>
  </div>
...
<div style="background:white;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
  ...
  <div style="padding:24px;">
    ...
    // 3. Log to Airtable
    await logToAirtable(data).catch((e: any) => console.error('Airtable log failed:', e));
