...
- **Slice 1 (2026-07-11):** `api/cmo-guards.ts` shared module (kill switch, per-IP/per-email rate limits, global daily budget, disposable blocklist, email-vs-website plausibility, admin-key gate, throttled ops alert; Upstash-backed, no-op without `UPSTASH_*`). Wired into `cmo-free-start`, `cmo-free-qualify`, `cmo-free-report`. Public `force` removed; manual report HTTP handler now requires `CMO_ADMIN_KEY`; wildcard CORS dropped.
- **Hotfix (2026-07-13):** Phase 0 merge briefly 500'd the whole funnel in prod. Root cause: `package.json` is `type: module`, so `api/*.js` run as ESM and relative imports MUST carry `.js` extensions. `cmo-guards.ts` started life as `_cmo-guards.ts` (underscore was a red herring) and the imports were extensionless. Fixed by adding `.js` to all five api relative imports (this also repaired a pre-existing extensionless `cmo-free-qualify` -> `cmo-free-report` import). Guards added: `scripts/check-api-imports.sh` (static lint, runs in the test suite) and `scripts/smoke-preview.sh <url>` (deploy smoke test). Rule: verify any api relative-import change on a Vercel PREVIEW before merging. See memory `reference_treetopnest_api_esm`.
